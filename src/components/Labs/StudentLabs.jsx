...
export default function StudentLabs() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1440px] w-full mx-auto space-y-4 sm:space-y-6 animate-in fade-in duration-300 flex flex-col flex-1 min-h-[calc(100vh-64px)]">
      ...
      {openLab ? (
        <LabDetail lab={openLab} onBack={() => setOpenLab(null)} />
      ) : (
        <>
                ...
                {filtered.map(lab => {
                  ...
                  return (
                    <button key={lab.id} onClick={() => setOpenLab(lab)}
                      ...
                      <h3 className="font-semibold text-zinc-900 text-base leading-tight line-clamp-1 mt-3 group-hover:text-primary transition-colors">{lab.title}</h3>
      ...
      <div className="bg-white rounded-lg ring-1 ring-black/5 shadow-sm p-5 sm:p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-semibold text-zinc-900 tracking-tight">{lab.title}</h2>
            <div className="flex flex-wrap items-center gap-2 mt-1.5 text-[11px] sm:text-xs font-medium text-zinc-500">
      ...
      {ordered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {ordered.map(r => {
            ...
            return (
              <div key={r.id}
                className="bg-white rounded-md ring-1 ring-black/5 shadow-sm p-4 flex flex-col sm:flex-row sm:items-center gap-4">
