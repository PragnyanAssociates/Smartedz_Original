...
const [downloading, setDownloading] = useState(false);
  const downloadXlsx = async () => {
    ...
    setDownloading(true);
    try {
      const qs = new URLSearchParams(); // Fixed typo here
        ...
        <div className="flex flex-col gap-1 ml-auto">
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Voucher, name, head..."
              className="h-9 w-52 rounded border border-zinc-200 shadow-sm bg-white pl-7 pr-2 text-xs text-zinc-700 outline-none focus:ring-1 focus:ring-primary/40" />
          </div>
        </div>
        <button onClick={downloadXlsx} disabled={!rows.length || downloading} title="Download this register as an Excel file"
          className="inline-flex items-center justify-center gap-1.5 bg-primary text-white h-9 px-4 shrink-0 rounded-md text-xs font-semibold hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-50">
      ...
      <div className="ring-1 ring-black/5 shadow-sm rounded-lg bg-white overflow-hidden">
        <div className="p-4 border-b border-zinc-100 flex items-center justify-between gap-3 flex-wrap">
          <h3 className="text-sm font-semibold text-zinc-900 flex items-center gap-2">
            <ListChecks className="size-4 text-primary" /> Voucher Register <span className="text-zinc-500 font-normal">({filtered.length})</span>
            {yearLabel && <span className="text-[10px] font-semibold text-blue-700 bg-blue-50 ring-1 ring-inset ring-blue-600/20 px-2 py-0.5 rounded-full uppercase tracking-wider">{yearLabel}</span>}
          </h3>
          <div className="flex items-center gap-4">
            <span className="text-[11px] text-zinc-500">Total: <strong className="text-zinc-900 tabular-nums">INR {fmtAmt(total)}</strong></span>
            {canEdit && (
              <button onClick={onNew} className="inline-flex items-center justify-center gap-1.5 h-9 px-4 shrink-0 bg-white text-zinc-600 border border-zinc-200 shadow-sm rounded-md text-xs font-semibold hover:bg-zinc-50 hover:text-primary transition-colors">
              ...
              <tbody className="divide-y divide-zinc-100">
                {filtered.length ? filtered.map((r, i) => (
                  <tr key={r.id} className="hover:bg-zinc-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-zinc-900">{r.name_title || '-'}</td>
                    <td className="px-4 py-3 text-xs text-zinc-600">{r.account_type || '-'}</td>
                    <td className="px-4 py-3 text-sm font-semibold text-zinc-900 tabular-nums whitespace-nowrap">INR {fmtAmt(r.total_amount)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        <button onClick={() => setViewId(r.id)} title="View" className="flex items-center justify-center size-8 bg-white text-zinc-600 border border-zinc-200 shadow-sm rounded-md hover:text-primary hover:bg-zinc-50 transition-colors"><Eye className="size-4" /></button>
                        {canEdit && <button onClick={() => onEdit(r.id)} title="Edit" className="flex items-center justify-center size-8 bg-white text-zinc-600 border border-zinc-200 shadow-sm rounded-md hover:text-primary hover:bg-zinc-50 transition-colors"><Pencil className="size-4" /></button>}
                        {canEdit && <button onClick={() => del(r.id)} disabled={busyId === r.id} title="Delete" className="flex items-center justify-center size-8 bg-white text-zinc-600 border border-zinc-200 shadow-sm rounded-md hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-40"><Trash2 className="size-4" /></button>}
                      </div>
                    </td>
                  </tr>
                )) : (
