    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-lg ring-1 ring-black/5 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="bg-primary text-white px-5 py-3 flex items-center justify-between sticky top-0">
          <div>
            <p className="text-sm font-bold">{v?.voucher_no || 'Voucher'}</p>
            <p className="text-[10px] opacity-85">Debit Voucher · {fmtVoucherDate(v?.voucher_date)}</p>
          </div>
        ...
        {loading || !v ? (
          ...
          <>
            <div className="p-5 space-y-3 text-sm">
              ...
              {/* Audit */}
              <div className="rounded-md bg-zinc-50 ring-1 ring-zinc-100 p-3 space-y-1.5 mt-2">
                <div className="flex items-center gap-2 text-[11px] text-zinc-500">
                  <Clock className="size-3.5 text-primary shrink-0" />
                  <span>Created by <strong className="text-zinc-700">{v.created_by_name || 'Unknown'}</strong> · {fmtISTDateTime(v.created_at)}</span>
                </div>
                {v.updated_at && (
                  <div className="flex items-center gap-2 text-[11px] text-zinc-500">
                    <UserCog className="size-3.5 text-accent shrink-0" />
                    <span>Updated by <strong className="text-zinc-700">{v.updated_by_name || 'Unknown'}</strong> · {fmtISTDateTime(v.updated_at)}</span>
                  </div>
                )}
              </div>
