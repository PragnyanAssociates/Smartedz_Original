    ...
    <div className="space-y-6">
      <div className="bg-blue-50/60 border border-blue-100 rounded-md p-4 flex items-start gap-3 text-[11px] text-blue-800 leading-relaxed">
        ...
        <p>
          ...
          <strong className="font-semibold"> Full Fee - Concession</strong>. Set the amount to <strong className="font-semibold">0</strong> to remove a concession.
          A concession applies to the <strong className="font-semibold">selected academic year only</strong> - next year the student starts on
      ...
      <div className="ring-1 ring-black/5 rounded-lg bg-white overflow-hidden shadow-sm">
        <div className="p-4 border-b border-zinc-100 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-zinc-900">
            Concessions <span className="text-zinc-500 font-normal">({students.length})</span>
          </h3>
                  ...
                  <tr key={s.id} className="hover:bg-zinc-50/60 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <input type="number" min="0" value={valueFor(s, 'amount')} disabled={!canEdit}
                        ...
                        placeholder="0"
                        className="w-28 rounded-md border border-zinc-200 shadow-sm bg-white px-2 h-8 text-xs text-zinc-900 tabular-nums focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 disabled:bg-zinc-50 disabled:text-zinc-400" />
                    </td>
                    <td className="px-5 py-3">
                      <input value={valueFor(s, 'reason')} disabled={!canEdit}
                        ...
                        className="w-44 rounded-md border border-zinc-200 shadow-sm bg-white px-2 h-8 text-xs text-zinc-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 disabled:bg-zinc-50 disabled:text-zinc-400" />
                    </td>
                    <td className="px-5 py-3 text-xs font-semibold tabular-nums">
                      {amount > 0
                        ? <span className="text-zinc-900">{inr(fullFeeNum)} <span className="text-amber-600">- {inr(amount)}</span> = <span className="text-primary">{inr(net)}</span></span>
                        : <span className="text-zinc-900">{inr(net)}</span>}
                    </td>
                    <td className="px-5 py-3 text-right">
                      {canEdit && (
                        <button onClick={() => saveConcession(s)} disabled={savingId === s.id || !dirty}
                          className="inline-flex items-center justify-center gap-1.5 h-8 px-3 rounded-md text-xs font-medium transition-colors bg-primary text-white shadow-sm hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed">
