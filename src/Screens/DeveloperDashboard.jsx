...
import { Building2, Plus, LogOut, Trash2, Edit3, Image as ImageIcon, Shield, LayoutGrid, Mail, Lock, User, Globe, Phone } from 'lucide-react';
...
export default function DeveloperDashboard() {
  ...
  const openEditModal = (inst) => {
    ...
    setFormData({ 
      name: inst.name, type: inst.type, logo: inst.logo || '', school_email: inst.school_email || '', phone: inst.phone || '',
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      ...
      <main className="p-8 max-w-[1400px] mx-auto w-full">
        <div className="flex justify-between items-center mb-10">
            ...
            <button 
              onClick={() => { setIsEditMode(false); setFormData({name:'', type:'School', logo:'', school_email: '', phone: '', superAdminName:'', superAdminEmail:'', superAdminPassword:''}); setIsModalOpen(true); }} 
              className="bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-200 px-8 py-4 rounded-2xl transition-all hover:scale-105 active:scale-95 font-bold flex items-center"
            >
            ...
            <div key={inst.id} className="group border-none shadow-sm hover:shadow-2xl transition-all duration-500 rounded-[2rem] overflow-hidden bg-white ring-1 ring-slate-100 flex flex-col">
              <div className="flex flex-row justify-between items-center bg-slate-50/50 p-6 border-none">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { if(window.confirm('Are you sure you want to delete this institution?')) fetch(`${API_URL}/api/developer/institution/${inst.id}`, {method:'DELETE'}).then(refreshData) }} 
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <input required placeholder="Lincoln High" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50/50" />
                    </div>
                    <div className="space-y-1">
                        <label className="block text-xs font-semibold text-slate-700 mb-1 ml-1">Category</label>
                        <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50/50 cursor-pointer appearance-none">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <input required type="email" placeholder="info@school.com" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50/50 pl-10" value={formData.school_email} onChange={e => setFormData({...formData, school_email: e.target.value})} />
                        </div>
                    </div>
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <input required placeholder="+91 000-000-0000" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50/50 pl-10" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                        </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50/80 p-6 rounded-[2rem] border border-slate-100 space-y-4 relative overflow-hidden">
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <input required placeholder="Full Name" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white pl-12" value={formData.superAdminName} onChange={e => setFormData({...formData, superAdminName: e.target.value})} />
                    </div>
                    <div className="relative">
                      <Mail className="absolute left-4 top-4 text-slate-400" size={16} />
                      <input type="email" required placeholder="Admin Login Email" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white pl-12" value={formData.superAdminEmail} onChange={e => setFormData({...formData, superAdminEmail: e.target.value})} />
                    </div>
                    <div className="relative">
                      <Lock className="absolute left-4 top-4 text-slate-400" size={16} />
                      <input required placeholder="Login Password" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white pl-12" value={formData.superAdminPassword} onChange={e => setFormData({...formData, superAdminPassword: e.target.value})} />
                    </div>
                  </div>
                </div>
              </div>
