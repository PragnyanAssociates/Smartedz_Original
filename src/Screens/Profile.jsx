...
export default function Profile() {
  ...
  const handleSave = async (e) => {
    ...
    // Phone validation (same rule as the admin form): 10 digits, no leading 0
    if (form.phone_no && !/^[1-9][0-9]{9}$/.test(form.phone_no)) {
      return alert('Phone number must be exactly 10 digits and cannot start with 0.');
    }
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE_URL}/profile/${user.id}`, {
...
function DisplayView({ profile, onEdit }) {
  // Role buckets decide which extra sections to show
  const isStaff   = profile.role === 'Super Admin' || (profile.role || '').toLowerCase().includes('teacher');
  const isStudent = (profile.role || '').toLowerCase().includes('student');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {/* Staff (Super Admin / Teacher) — read-only, managed by admin */}
        {isStaff && (
          <Section title="Employment Details">
            <Row label="Aadhaar Number" value={profile.aadhar_no} />
            <Row label="Joining Date" value={fmtDMY(profile.joining_date)} />
            <Row label="Experience" value={profile.experience} />
            <Row label="Previous Salary" value={fmtMoney(profile.prev_salary)} />
            <Row label="Present Salary" value={fmtMoney(profile.present_salary)} />
          </Section>
        )}

        {/* Student — read-only, managed by admin */}
        {isStudent && (
          <Section title="Student Details">
            <Row label="PEN Number" value={profile.pen_no} />
            <Row label="Aadhaar Number" value={profile.aadhar_no} />
            <Row label="Parent / Guardian" value={profile.parent_name} />
            <Row label="Roll Number" value={profile.roll_no} />
            <Row label="Admission Number" value={profile.admission_no} />
            <Row label="Admission Date" value={fmtDMY(profile.admission_date)} />
            <Row label="School Joined Date" value={fmtDMY(profile.school_joined_date)} />
            <Row label="School Joined Grade" value={profile.school_joined_grade} />
            <Row label="TC Number" value={profile.tc_number} />
          </Section>
        )}
      </div>
    </div>
    ...
    <form onSubmit={onSave} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        <Section title="Personal Information">
          ...
          <FieldRow>
            <Field label="Date of Birth" type="date" value={form.dob} onChange={set('dob')} icon={Calendar} hint="DD/MM/YYYY" />
        ...
        <Section title="Contact Information">
          <FieldRow>
            ...
            <Field label="Phone Number" value={form.phone_no} onChange={setPhone} icon={Phone}
              inputMode="numeric" maxLength={10} hint="10 digits, cannot start with 0" />
          </FieldRow>
