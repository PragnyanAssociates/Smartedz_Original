...
export default function PermissionsTab({ data }) {
  ...
  const [matrix, setMatrix]                 = useState({});   // { moduleName: { can_read, can_edit, can_delete, is_hidden } }
  const [saving, setSaving]                 = useState(false);
  const [loadingPerms, setLoadingPerms]     = useState(false);

  // Build a fresh matrix for the selected role
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm font-medium mt-1">
            Pick a role, then decide which modules they can see, read, edit or delete.
          </p>
        </div>
      ...
      {!selectedRoleId ? (
        <div className="bg-white p-16 rounded-3xl border border-dashed border-slate-200 text-center">
          ...
          <p className="text-slate-400 font-bold">Select a role to configure its permissions.</p>
        </div>
      ) : loadingPerms ? (
        ...
        <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
          <div className="px-6 py-4 bg-blue-50/50 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-blue-100">
              <Save size={16} />
          ...
          <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 text-xs text-slate-500 font-medium">
            <strong className="text-slate-700">Rules:</strong> Toggling <em>Hide</em> automatically clears the others — hidden modules don't appear in the sidebar at all. Granting <em>Edit</em> or <em>Delete</em> auto-enables <em>Read</em>.
          </div>
        </div>
      )}
    </div>
